...
author: 
  name: "Joshua Wright, Ph.D."
  affiliation: Office of Institutional Research
format: 
  html:
...
# Here I create a data frame that contains the known graduation rates from 2016-2022.

data <- data.frame(year = c(2016, 2017, 2018, 2019, 2020, 2021, 2022), grad_rate = c(.28, .32, .41, .37, .42, .4, .37))

# I then create a new data frame which contains the unknown years 2023-2025
...
# Here I reproduce the original analyst's model, which will be the base model. We model graduation rate as a function of year using the 2016-2022 data. 

base_model <- lm(grad_rate ~ year, data = data)

# If we use summary() to review the base model, we can see that year accounts for approximately 33% of the variability in graduation rates.

# Here I use the predict function to produce the estimates for 2016-2022 using the base model, which ultimately is the formula GradRate = -31.36 + year*0.0157. 
#This also includes the prediction interval for understanding the potential range of possible values we could expect.

pred.int <- data.frame(predict(base_model, interval = 'prediction'))

#Because when I plot the model for stakeholders, I want to show the model, the known data for 2016-2022, and the predictions for 2023-2025, I need to combine my model predictions with the origina data using a cbind
...
# The plot the primary reference for the stakeholders, which will demonstrate the model itself, the known graduates rates from 2016-2022, and the projections for 2023-2025. We include the lower and upper bounds of the predictions to visualize the uncertainty. 
...
# My biggest gripe about this model is that it does not account for any internal or external variables that plausibly impact graduation rates. 
```
...
# The next model with take into account other variables that potentially impact graduation rates. First, I'll create the dataframe we'll use to built the model.
...
# Here we create our revised model to include the new variables
better_model <- lm(grad_rate ~ texas_grad_rate + sat_math_cohort + unemployment, data = data)

# If we evaluate the model, we can see that this set of variable accounts for approximately 62% of variability in our college's graduation rates.
...
# We then combine the unknown years to the known years and pull the predicted values for 2023-2025 into the graduation rates column so that all graduation rates are in the same column. We know have a dataframe that contains 2016-2025, including known college graduation rates for 2016-2022 and predicted college graduation rates for 2023-2025.
...
# Finally, we plot our model, with the predictions, and the lower and upper bounds of the predictions. 
